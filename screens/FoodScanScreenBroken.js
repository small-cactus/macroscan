...
const stopLoadingAnimation = () => {
    ...
    try {
      ...
      if (selectedMode === SEARCH_MODE) {
        ...
        if (!isSubscribed && !isFirstDayUnlimited) {
          // Use current date instead of getTodayString
  ...
  const renderDetailsTab = () => {
    ...
    // Get the summary text from various possible sources
    const summaryText = foodData?.details?.summaryText || 
                       foodData?.details?.summary || 
                       (foodData?.food?.name ? `Nutritional information for ${foodData.food.name}.` : '');
    
    const hasSummary = !!summaryText;

    // If no details and no search data, show empty state
    if (!hasDetails && !hasSearchData && !hasSummary) {
      return (
        <View style={styles.tabContentContainer}>
      ...
      <View style={styles.tabContentContainer}>
        ...
        >
          <View style={styles.macroGridContainer}>
              ...
              >
                ...
                <View style={styles.linksContainer}>
                  {foodData.details.sources.map((source, index) => (
                    ...
                    >
                      <Text style={styles.linkText}>{source.title || source.name || 'Source'}</Text>
                      {source.url && (
            ...
            {/* Search Results (from Search mode) */}
              ...
              >
                ...
                >
                  <View style={styles.iconLabelContainer}>
                    ...
                    <Text style={styles.macroLabel}>Web Results</Text>
                  </View>
                </TouchableOpacity>
                ...
                <View style={styles.searchResultsContainer}>
                  {searchResults.map((result, index) => (
                    <View key={index} style={styles.searchResultItem}>
                      <TouchableOpacity
                        onPress={() => result.url ? Linking.openURL(result.url) : null}
                        style={styles.searchResultTitle}
                      >
                        <Text style={styles.searchResultTitleText}>{result.title || 'Search Result'}</Text>
                        {result.url && (
                          <Icon name="open-outline" size={14} color={colorScheme === 'dark' ? '#BBBBBB' : '#666666'} />
                        )}
                      </TouchableOpacity>
                      {result.url && <Text style={styles.searchResultUrl}>{result.url}</Text>}
                      {result.snippet && <Text style={styles.searchResultSnippet}>{result.snippet}</Text>}
                    </View>
                  ))}
                </View>
              </Animated.View>
  ...
  const modeSelectionAlert = () => {
    ...
    const modeDescriptions = {
      ...
      search: 'Search Mode (BETA) allows you to search for food items directly.',
    };
  ...
  const handleSuccessfulScan = async (parsedData, imageUri, barcodeData, hasDrawing = false, actualModel = null) => {
    try {
      ...
      if (!parsedData) {
        console.error("Invalid parsedData structure: null or undefined");
        setNoFoodFound(true);
        return false;
      }
      
      // Extract search queries and results from _searchInfo if available (for search mode)
      if (parsedData._searchInfo) {
        console.log("Found search info in parsedData");
        if (parsedData._searchInfo.queries) {
          setSearchQueries(parsedData._searchInfo.queries);
        }
        if (parsedData._searchInfo.results) {
          setSearchResults(parsedData._searchInfo.results);
        }
      }
      ...
      if (parsedData.food) {
        // Standard format with food object
        console.log("Setting food data from parsedData.food");
        setFoodData(parsedData.food);
      } else if (parsedData.name) {
        // Direct food object (without food wrapper)
        console.log("Setting food data directly from parsedData");
        setFoodData(parsedData);
      } else {
        console.error("No valid food data found in parsedData:", parsedData);
        setNoFoodFound(true);
        return false;
      }
      ...
      await storeProductDetails({
        productName: parsedData.food ? parsedData.food.name : parsedData.name,
        imageUri: imageUri,
        nutrients: parsedData.food || parsedData,
        date: new Date().toISOString(),
  ...
  // Add search mode specific state variables
  const [searchQueries, setSearchQueries] = useState([]);
  const [searchResults, setSearchResults] = useState([]);
  ...
  const [searchVisualizationCompleted, setSearchVisualizationCompleted] = useState(false);
  const visualizationRef = useRef(null);
  const intervalIdsRef = useRef([]);
  
  // Add reference to track if user has already used their free search scan for the day
  const [freeSearchScanUsed, setFreeSearchScanUsed] = useState(false);

  // Add a function to check if the user has already used their free search scan for the day
  const checkFreeSearchScanAvailability = async () => {
    if (isSubscribed || isFirstDayUnlimited) {
      // Subscribed users have unlimited search scans
      return true;
    }

    try {
      // Get today's date directly
      const today = new Date();
      const todayString = today.toISOString().split('T')[0]; // Format as YYYY-MM-DD
      
      // Check if user has already used their free search scan today
      ...
      if (lastFreeSearchScanDate === todayString && freeSearchScansUsed === '1') {
        setFreeSearchScanUsed(true);
        return false;
      }
      
      // If it's a new day or they haven't used their scan yet, they can use it
      if (lastFreeSearchScanDate !== todayString) {
        // Reset the count for the new day
        await AsyncStorage.setItem('freeSearchScansUsed', '0');
      }
      
      setFreeSearchScanUsed(false);
      return true;
    } catch (error) {
  ...
  const handleSearchModeScan = async (base64Image, imageUri, barcodeData, hasDrawing) => {
    try {
      ...
      const finishSearchProcess = async (parsedData) => {
        console.log('Finalizing search process with data:', parsedData?.food?.name || "unknown food");
        
        // Make sure we have complete food data
        if (!parsedData.food && detectedFoodItems.length > 0) {
          parsedData.food = { 
            name: detectedFoodItems[0],
            class: "Food",
            type: "Unknown",
        ...
        if (visualizationRef.current) {
          ...
          setTimeout(() => {
            if (visualizationRef.current && visualizationRef.current.completeVisualization) {
              ...
              // Add a longer delay before hiding the visualization
              setTimeout(() => {
                console.log('Hiding visualization after delay');
                setShowVisualization(false);
              }, 4000); // Increased to 4 seconds for better visibility
            }
          }, 800);
      ...
      const searchTrackingHandler = (queries, results) => {
        // Update search queries and results state
        if (Array.isArray(queries) && queries.length > 0) {
          console.log('Got search queries:', queries);
          ...
          // Try to extract food items from queries to improve visualization
          if (queries.length > 0 && detectedFoodItems.length === 0) {
            const foodKeywords = ['nutrition', 'calories', 'food', 'recipe', 'facts for'];
            const possibleFoodItems = [];
            
            queries.forEach(query => {
              // Look for "nutrition facts for X" pattern
              const factsForMatch = query.match(/(?:nutrition|nutrient)\s+(?:facts|information)\s+(?:for|about)\s+(.+?)(?:$|\.|,)/i);
              if (factsForMatch && factsForMatch[1]) {
                possibleFoodItems.push(factsForMatch[1].trim());
              }
            });
            
            if (possibleFoodItems.length > 0) {
              setDetectedFoodItems(possibleFoodItems);
            }
          }
        }
        ...
        if (Array.isArray(results) && results.length > 0) {
          ...
          setSearchResults(prev => {
            return [...prev, ...results.filter(r => !prev.some(existing => existing.url === r.url))];
          });
        }
        
        // Update the visualization component with new data in real-time
        if (visualizationRef.current) {
          // Create a temporary data object for visualization updates
          const updateData = { 
            food: detectedFoodItems.length > 0 ? { name: detectedFoodItems[0] } : { name: "Food" },
            _searchInfo: {
              queries: searchQueries,
              results: searchResults
            }
          ...
          // Force subtitle updates to show real-time progress
      ...
      const providerParams = {
        ...
        handleSuccessfulScan: async (parsedData, imageUri, barcodeData, hasDrawing, actualModel) => {
          try {
            console.log('Processing successful scan with data:', parsedData?.food?.name || "unknown food");
            
            // Process and store food data
            if (parsedData && (parsedData.food || parsedData.name)) {
              // For search mode, we need to handle both formats
              const foodData = parsedData.food || parsedData;
              ...
              if (foodData.name) {
                setDetectedFoodItems(prev => {
                  if (!prev.includes(foodData.name)) {
                    return [...prev, foodData.name];
                  }
                  return prev;
                });
              }
      ...
      const foodFound = await handleWebSearch(providerParams);
      return foodFound;
    } catch (error) {
      console.error('Error handling search mode scan:', error);
  ...
  // Add this helper function to extract food items for visualization
  const getFoodItems = () => {
    const items = [];
  ...
  // Add a new renderSearchTab function to display search results
    ...
    return (
      ...
      >
        <View style={styles.searchContainer}>
          ...
          {searchResults.length > 0 && (
            <View style={styles.searchResultsContainer}>
              <Text style={styles.subsectionTitle}>Source Information</Text>
              {searchResults.map((result, index) => (
                <View key={index} style={styles.searchResultItem}>
  ...
  // Modify the existing tabs to include Search tab
  // Update in the renderTabBar or similar function where tabs are defined
  const renderTabBar = () => {
    // Get tabs based on scan mode and results
    const tabs = ['Nutrition', 'Ingredients', 'Details']; 
    
    // Add Search tab when we have search data or in search mode
    if (selectedMode === SEARCH_MODE || searchQueries.length > 0 || searchResults.length > 0) {
      tabs.push('Search');
    }
  ...
  // Modify the existing renderTabContent function to include the search tab
  ...
  // Add effect to update tab selection when food data changes
  useEffect(() => {
    // When food data becomes available, make sure a tab is selected
    if (foodData && !activeTab) {
      setActiveTab('Nutrition');
      
      // If we have search data, make the Search tab visible
      if ((searchQueries && searchQueries.length > 0) || 
          (searchResults && searchResults.length > 0) ||
          selectedMode === SEARCH_MODE) {
        // Update tabIndicator position to handle 4 tabs
        setTimeout(() => {
          // Animate tab indicator to Nutrition position
          Animated.spring(tabIndicatorAnim, {
            toValue: 0, // Nutrition tab
            useNativeDriver: true,
            friction: 24,
            tension: 180,
            velocity: 10
  ...
  }, [foodData, activeTab, searchQueries, searchResults, selectedMode]);
    ...
    >
      <View style={styles.headerContainer}>
        <AnimatedTextFoodScan
          text={
            foodData
              ? (foodData.food ? 
                  (foodData.food.name.length > 23
                    ? foodData.food.name.slice(0, 23) + '...'
                    : foodData.food.name) 
                : foodData.name 
      ...
      <AnimatedTextFoodScan
        text={
          foodData
            ? foodData.food 
              ? `${foodData.food.class || 'Unknown Class'} • ${foodData.food.type || 'Unknown Type'}`
            ...
            >
              ...
              >
                <View style={styles.chipContent}>
                  <Icon
                    name={
                      selectedMode === FAST_MODE ? 'flash' : 
                      selectedMode === ACCURATE_MODE ? 'shield-checkmark' :
                      'search'
                    }
                    size={16}
        ...
        {foodData ? (
          ...
          >
            <Animated.View style={[
              { 
                opacity: tabFadeAnim,
                transform: [{ translateY: tabFadeAnim.interpolate({
                  inputRange: [0, 1],
                  outputRange: [10, 0]
                })}]
              }
      ...
      >
        ...
        >
          <View style={styles.loadingCard}>
            ...
            <View style={styles.modeBadgeContainer}>
              <BlurView intensity={50} style={styles.modeBadge}>
                ...
                <Text style={styles.modeBadgeText}>
                  {
                    selectedMode === FAST_MODE ? 'Fast Scan' : 
                    selectedMode === ACCURATE_MODE ? 'Accurate Scan' : 
                    'Search Scan'
                  }
                </Text>
              </BlurView>
            ...
            {/* Loading Indicator or Visualization based on mode */}
            {selectedMode === SEARCH_MODE && showVisualization ? (
              <View style={styles.visualizationWrapper}>
                <FunctionalAIVisualization
                  ...
                  onComplete={() => {
                    console.log('Visualization complete callback triggered from modal');
                    // Don't hide loading immediately, let the parent component handle this
                  }}
                />
              </View>
            ) : (
              <View style={styles.loadingContent}>
                ...
                <Animated.View style={[styles.loadingTextContainer, { opacity: fadeAnim }]}>
                  <Text style={styles.loadingText}>{currentLoadingText}</Text>
                </Animated.View>
              </View>
            )}
