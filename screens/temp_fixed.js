...
const stopLoadingAnimation = () => {
  ...
  const sendImageToApi = async (imageUri, barcodeData = null, hasDrawing = false) => {
    ...
    try {
      ...
      if (selectedMode === SEARCH_MODE) {
        ...
        if (searchVisualizationRef.current && searchVisualizationRef.current.reset) {
          searchVisualizationRef.current.reset();
        }
        
        // Force visualization to be visible
        setShowSearchVisualization(true);
        
        // No need for delays or additional checks - keep it simple
      } else {
      ...
      // Search mode - use web search functionality
        ...
        await handleSearchModeScan(base64Image, imageUri, barcodeData, hasDrawing);
      }
  ...
  const [showSearchVisualization, setShowSearchVisualization] = useState(false);
  const searchVisualizationRef = useRef(null);
  ...
  const handleSearchModeScan = async (base64Image, imageUri, barcodeData, hasDrawing) => {
    try {
      ...
      if (!apiKey) {
        ...
        setIsLoading(false);
        stopLoadingAnimation();
        setShowSearchVisualization(false);
        isProcessingRef.current = false;
        return false;
      }
      
      // Update the visualization with food recognition stage
      if (searchVisualizationRef.current) {
        // Initial update immediately
        searchVisualizationRef.current.forceUpdateSubtitles();
        
        // Start a regular update interval to keep visualization fresh
        const updateInterval = setInterval(() => {
          if (searchVisualizationRef.current && isLoading) {
            // Create a combined update with all available data
            const updateData = {
              _searchInfo: {
            ...
            if (foodItems.length > 0) {
              updateData.food = { name: foodItems[0] };
            }
            
            searchVisualizationRef.current.updateWithScanData(updateData);
            searchVisualizationRef.current.forceUpdateSubtitles();
          } else {
      ...
      // First stage: Food recognition - Simulate haptic feedback
      Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
      
      // Prepare the callback functions for updating visualization
      const updateFns = {
        updateFoodItems: (items) => updateFoodItems(items),
        updateSearchQueries: (queries) => updateSearchQueries(queries),
        updateSearchResults: (results) => updateSearchResults(results),
        updateVisualization: (data) => {
          if (searchVisualizationRef.current) {
            searchVisualizationRef.current.updateWithScanData(data);
          }
        }
      };
      
      // Process the image through the web search provider
      const providerParams = {
        provider,
        selectedModel: currentModel,
        selectedMode: SEARCH_MODE,
        ...
        handleSearchTracking: (data) => {
          if (typeof data === 'object') {
            ...
            if (searchVisualizationRef.current) {
              searchVisualizationRef.current.updateWithScanData(combinedData);
              searchVisualizationRef.current.forceUpdateSubtitles();
            }
          }
  };
  ...
  const updateSearchResults = (results) => {
    ...
    if (searchVisualizationRef.current) {
      searchVisualizationRef.current.updateWithScanData({ _searchInfo: { results } });
    }
  };
  ...
  const renderSearchVisualization = () => {
    console.log('DEPRECATED: renderSearchVisualization should not be called anymore. Using modal visualization instead.');
    if (!isLoading || selectedMode !== SEARCH_MODE) {
      console.log('Not rendering visualization: isLoading=', isLoading, 'selectedMode=', selectedMode);
      return null;
    }
    
    console.log('Rendering search visualization with showSearchVisualization=', showSearchVisualization);
    console.log('Current data - queries:', searchQueries.length, 'results:', searchResults.length, 'foodItems:', foodItems.length);
    ...
    return (
      <View style={styles.visualizationContainer}>
        <FunctionalAIVisualization
          ...
          isVisible={showSearchVisualization} 
          ...
          onComplete={() => {
            ...
            // Let visualization finish then hide loading
            setTimeout(() => {
              setIsLoading(false);
              setShowSearchVisualization(false);
            }, 1500);
  ...
  useEffect(() => {
    ...
    if (selectedMode !== SEARCH_MODE && searchVisualizationRef.current) {
      ...
      if (searchVisualizationRef.current.reset) {
        searchVisualizationRef.current.reset();
      }
      setShowSearchVisualization(false);
    }
        ...
        <View style={styles.imageContainer}>
          {showPlaceholder ? (
            ...
            <View style={styles.imageWrapper}>
              ...
              />
              {/* Add the loading overlay with search visualization when needed */}
              {isLoading && (
          ...
          <View style={styles.controlsOverlay}>
                  ...
                  {isFirstDayUnlimited || isSubscribed ? (
                    ...
                    >
                      {selectedMode === ACCURATE_MODE
                        ...
                        : selectedMode === SEARCH_MODE
                          ? Math.max(0, 1 - freeSearchScansUsed)
                          : isSubscribedPlus
                            ? (20 - scanCount)
                            : (2 - scanCount)}
                    </Text>
            ...
            >
              <BlurView
                ...
                style={[
                  styles.chip,
                  selectedMode === ACCURATE_MODE && styles.chipAccurate,
                ...
                <View style={styles.chipContent}>
                  <Icon
                    name={
                      ...
                      selectedMode === SEARCH_MODE ? 'search' : 'flash'
                    }
                    size={16}
                  ...
                  >
                    {selectedMode === FAST_MODE ? 'Fast' : 
                     selectedMode === ACCURATE_MODE ? 'Accurate' : 
                     selectedMode === SEARCH_MODE ? 'Search' : 'Fast'}
                  </Animated.Text>
                </View>
              </BlurView>
            </TouchableOpacity>
...
>
  ...
  >
    <View style={styles.loadingCard}>
      ...
      {selectedMode === SEARCH_MODE && showSearchVisualization ? (
        <View style={styles.visualizationWrapper}>
          <FunctionalAIVisualization
            ...
            onComplete={() => {
              ...
              // Add a short delay before hiding the loading state
              setTimeout(() => {
                setIsLoading(false);
                setShowSearchVisualization(false);
              }, 2000);
...
const scale = Math.min(scaleWidth, scaleHeight);
  ...
  const getDynamicStyles = (colorScheme) => StyleSheet.create({
    ...
    // New styles for search content in details tab
    ...
    searchQueryItem: {
      ...
      alignItems: 'center',
      padding: 8 * scale,
      backgroundColor: colorScheme === 'dark' ? 'rgba(45, 45, 45, 0.6)' : 'rgba(240, 240, 240, 0.6)',
    ...
    // Style for the search visualization overlay
    ...
    // New style for the visualization in the loading modal
